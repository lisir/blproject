...
function onAddClick(){
    ...
    $('#mainDiv').dialog({
        ...
        buttons:[{
            ...
            handler:function(){
                //活动名称验证
                var activityName = $("#activityBean\\.activityName").val();
                if(!activityName){
                    $.messager.alert('输入错误','请输入活动名称!', 'error');
                    return;
                }
...
function onEditClick(){
    ...
    if (row){
        showMasker2('fullbg','dialog','dialog_content','');
        //要编辑的信息（编辑前）
        $("#activityBean\\.activityName").val(row.activityName);
        $("#activityBean\\.activityDesc").val(row.activityDesc);
        $("#activityBean\\.activityType").val(row.activityType);
        $("#activityBean\\.id").val(row.id);//主键
        $("#activityFlooradd").combobox('setValue',row.activityFloor);
        //显示div
        $("#mainDiv").css("display","block");
        $('#mainDiv').dialog({
            title:'计算机信息编辑',
            buttons:[{
                ...
                handler:function(){
                    ...
                    //活动类型验证
                    var activityType = $("#activityBean\\.activityType").val();
                    if(!activityType){
                        $.messager.alert('输入错误','请输入活动类型!', 'error');
                        return;
                    }
                   $("#activityBean\\.activityFloor").val($("#activityFlooradd").combobox('getValue'));
                    $('#mainDiv').dialog('close');
                    closeMasker('fullbg','dialog','dialog_content');
...
function onDeleteClick(){
    ...
    if (row){
    	 $.messager.confirm('计算机信息删除', '确认删除该计算机信息?', function(r){
             if (r){
			        $.post("./deleteActivity.action",{"activityBean.id":row.id},function(data){
			            if(!data.result){
			                alert(data.cause);
			            }else{
			            	alert("删除成功!");
        ...
        $.messager.alert("删除错误","请选择要删除的活动！！", 'error');
    }
}
...
function onInstallSoftwareClick(){
    var row = $('#maintable').datagrid('getSelected');
    if (row){
        //$("#activityBean\\.cid").val(row.cid);//主键
