function My97DP() {
	...
	A();
	$dp.focusArr = [document, $d.MI, $d.yI, $d.HI, $d.mI, $d.sI, $d.clearI,
			$d.todayI, $d.okI];
	for (var B = 0; B < $dp.focusArr.length; B++) {
		var _ = $dp.focusArr[B];
		_.nextCtrl = B == $dp.focusArr.length - 1
				? $dp.focusArr[1]
				: $dp.focusArr[B + 1];
...
function getWeek($, B) {
	...
	_.setMonth(0);
	_.setDate(4);
	return Math.round((A - _.valueOf()) / 604800000) + 1;
}
...
function _tab(M) {
	...
	if ($dp.enableInputMask && !$OPERA && !$dp.readOnly
		...
		var T = $dp.el, S = T.value, F = E(T), I = {
			str : "",
			arr : []
		}, R = 0, K, N = 0, X = 0, O = 0, J, _ = /yyyy|yyy|yy|y|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|WW|W|w/g, L = $dp.dateFmt
				.match(_), B, A, $, V, W, G, J = 0;
		if (S != "") {
			O = S.match(/[0-9]/g);
			O = O == null ? 0 : O.length;
		...
		while ((K = _.exec($dp.dateFmt)) !== null) {
			...
			if (I.arr[X + 1] || A == "M" && W > 12 || A == "d" && W > 31
					|| A == "H" && W > 23 || "ms".indexOf(A) >= 0 && W > 59) {
				if (K[0].length == 2) {
					B = "0" + $;
				} else {
					B = $;
...
My97DP.prototype = {
	...
	splitDate : function(J, C, Q, E, B, G, F, K, L) {
		...
		if (J && J.loadDate) {
			...
			if (J != "") {
				C = C || $dp.dateFmt;
				var H, P = 0, O, A = /yyyy|yyy|yy|y|MMMM|MMM|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|DD|D|WW|W|w/g, _ = C
						.match(A);
				A.lastIndex = 0;
				if (L) {
					O = J.split(/\W+/);
		...
		function N(A, $) {
			...
			for (var B = 0; B < 12; B++) {
				if (_[B].toLowerCase() == $.substr(0, _[B].length)
						.toLowerCase()) {
					return B + 1;
				}
			}
	...
	loadDate : function() {
		var _, $;
		if ($dp.alwaysUseStartDate || $dp.startDate != ""
				&& $dp.el[$dp.elProp] == "") {
	...
	checkValid : function($, A, B) {
		...
		if (_ == 0) {
			...
			if (A == "d" && B == null) {
				B = Math
						.abs(((new Date($.y, $.M - 1, $.d)).getDay() - $dp.firstDayOfWeek)
								% 7);
			}
	...
	checkAndUpdate : function() {
		...
		if ($ != null) {
			...
			if ($ == "" || A.isDate(A.date) && A.isTime(A.date)
					&& A.checkValid(A.date)) {
				if ($ != "") {
	...
	_fd : function() {
		var E, C, D, K, A, H = new sb, F = $lang.aWeekStr, G = $dp.firstDayOfWeek, I = "", $ = "", _ = new DPDate(
		...
		for (E = 1, C = A; E < 7; E++) {
			...
			for (D = 0; D < 7; D++) {
				...
				if ($dp.isShowWeek && D == 0 && (E < 4 || K)) {
					H.a("<td class=Wweek>"
							+ getWeek(_, $dp.firstDayOfWeek == 0 ? 1 : 0)
							+ "</td>");
				}
				H.a("<td ");
				if (K) {
					if (this.checkValid(_, "d", D)) {
						if (this.testSpeDay(Math.abs(((new Date(_.y, _.M - 1,
								_.d)).getDay() - $dp.firstDayOfWeek)
								% 7))
								|| this.testSpeDate(_)) {
							I = "WspecialDay";
						}
	...
	getP : function(D, _, $) {
		...
		switch (D) {
			...
			case "M" :
				...
				F = function($) {
					return $.length == 4
							? $lang.aLongMonStr[A - 1]
							: $.length == 3 ? $lang.aMonStr[A - 1] : doStr(A,
									$.length);
				};
				break;
			default :
	...
	getDateStr : function(_, $) {
		...
		if ($dp.has.D) {
			...
			_ = this.getP("M", _, $);
			_ = _.replace(/\%dd/g, this.getP("D", "DD")).replace(/\%d/g,
					this.getP("D", "D"));
		} else {
			_ = this.getP("M", _, $);
		}
	...
	autoSize : function() {
		...
		for (var C = 0; C < _.length; C++) {
			...
			if (_[C].contentWindow == window && A) {
				...
				if (B && $d.bDiv.style.display == "none"
						&& $d.tDiv.style.display != "none"
						&& document.body.scrollHeight - A >= B) {
					A += B;
					$d.style.height = A;
				} else {
					$d.style.height = $;
